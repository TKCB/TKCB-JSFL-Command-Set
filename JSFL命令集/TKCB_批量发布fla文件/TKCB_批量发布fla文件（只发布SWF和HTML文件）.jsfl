...
function setPublishProfile ()
{
	...
	// SWF格式
	re = /\<flash\>0\<\/flash\>/i;
	newPPXML = newPPXML.replace(re, "<flash>1</flash>");

	// HTML格式
	re = /\<html\>0\<\/html\>/i;
	newPPXML = newPPXML.replace(re, "<html>1</html>");
	...
	// SWC格式
	re = /\<swc\>1\<\/swc\>/i;
	newPPXML = newPPXML.replace(re, "<swc>0</swc>");
	re = /\<ExportSwc\>1\<\/ExportSwc\>/i;
	newPPXML = newPPXML.replace(re, "<ExportSwc>0</ExportSwc>");
