...
if ( FLfile.exists(folderURI) )
{
	...
	fl.trace( "————共 " + list.length + " 个fla文件，重命名并整理所有资源完成！————" );
}

//// 批量设置函数
function batchSet ( doc )
{
var tlibrary = doc.library;			// 当前文档库
var tItems = doc.library.items;		// 当前文档库中所有元素（图片、元件、声音等等）
var tItem;						// 元素
var i;
...
//// 创建库文件夹
tlibrary.newFolder( "组件" );
tlibrary.newFolder( "字体" );
tlibrary.newFolder( "声音" );
tlibrary.newFolder( "图片" );
tlibrary.newFolder( "视频" );
tlibrary.newFolder( "元件" );
...
for ( i = 0; i < len; i++ )
{
	var myDate = new Date();
	tItem = tItems[ i ];
	if ( tItem.name != "组件" && tItem.name != "字体" && tItem.name != "图片" && tItem.name != "声音" && tItem.name != "视频" && tItem.name != "元件" )
	{
	...
	if ( tItem.itemType != "folder" )
	{
		...
		{
			case "component":
				tlibrary.moveToFolder( "组件", tItem.name );
				break;
			case "font":
				tlibrary.moveToFolder( "字体", tItem.name );
				break;
			case "bitmap":
				tlibrary.moveToFolder( "图片", tItem.name );
				break;
			case "sound":
				tlibrary.moveToFolder( "声音", tItem.name );
				break;
			case "video":
				tlibrary.moveToFolder( "视频", tItem.name );
				break;
			default:
				tlibrary.moveToFolder( "元件", tItem.name );
				break;
		}
	}
}
...
for ( i = 0; i < len; i++ )
{
	...
	if ( tItem.itemType == "folder" )
	{
		if ( tItem.name != "字体" && tItem.name != "元件" && tItem.name != "图片" && tItem.name != "声音" && tItem.name != "视频" && tItem.name != "组件" )
		{
...
{
	tItem = tItems[ i ];
	if ( tItem.name != "字体" && tItem.name != "元件" && tItem.name != "图片" && tItem.name != "声音" && tItem.name != "视频" && tItem.name != "组件" && tItem.itemType != "undefined" )
	{
		strPrefix = "";
