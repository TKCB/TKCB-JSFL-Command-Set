...
if ( FLfile.exists(folderURI) )
{
	...
	fl.trace( "————共 " + list.length + " 个fla文件，设置所有图片的类绑定完成！————" );
}
...
for ( i = 0; i < len; i++ )
{
	...
	//// 判断确定这个图片没有被绑定基类
        if( tItem.itemType == "bitmap" && tItem.linkageBaseClass == undefined && tItem.linkageExportForAS == false && tItem.linkageExportInFirstFrame == undefined )
        {
