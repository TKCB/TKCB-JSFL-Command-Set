...
fl.trace( "【TKCB提示】  使用此JSFL命令前，务必保证当前文件已经保存过，未命名的临时文件会导致报错无法运行此命令！！！" );
...
function main(){
	...
	prefix = info.name;
	var c_mask = info.cmask == "true"
	var c_guide = info.cguide == "true";
	var jumpEmptys = info.jumpEmptys == "true";
	var exportEmptys = info.exportEmptys =="true";
	...
	for(var i = 0;i<len;i++){
		...
		if(mode == 1){
			...
			fl.trace(frameRights[i]+" "+frameLefts[i]);
